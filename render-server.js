...
function seatIds(room) {
  return Array.from({ length: room.game.playerCount }, (_, i) => `P${i + 1}`);
}

function presence(room) {
  const out = {};
  for (const id of seatIds(room)) out[id] = room.sockets.get(id)?.readyState === WebSocket.OPEN;
  return out;
}

function isReady(room) {
  const online = presence(room);
  return seatIds(room).every((id) => room.players[id] && online[id]);
}

function findSeatByToken(room, token) {
  if (!token) return null;
  for (const id of seatIds(room)) {
    if (room.players[id]?.token === token) return { playerId: id, token };
  }
  return null;
}
...
function resolveSeat(room, token, name) {
  ...
  if (exact) {
    const p = room.players[exact.playerId];
    p.name = name || p.name;
  ...
  for (const id of seatIds(room)) {
    ...
    if (!existing) {
      ...
      room.players[id] = { token: newToken, name, disconnectedAt: null, connectedAt: null, joinReservedAt: now };
...
function retryAfterMs(room) {
  const now = Date.now();
  let best = RECLAIM_GRACE_MS;
  for (const id of seatIds(room)) {
    const p = room.players[id];
    if (!p) return 0;
    const base = p.disconnectedAt || p.joinReservedAt;
    if (base) best = Math.min(best, Math.max(0, RECLAIM_GRACE_MS - (now - base)));
  }
  return best;
}
...
function snapshot(room, forId = null) {
  ...
  for (const id of seatIds(room)) {
    const p = room.players[id];
    players[id] = p ? { name: p.name, connected: online[id] } : null;
  }
  return {
    roomCode: room.code,
    you: forId,
    ...
    serverNow: Date.now(),
    timerRevision: room.timerRevision || 0,
    backend: BACKEND,
  };
}
...
function broadcastState(room, extra = {}) {
  for (const [playerId, ws] of room.sockets) {
    send(ws, { type: 'state', ...snapshot(room, playerId), ...extra });
  }
}
...
function armTurnTimer(room, reset = false) {
  if (room.game.winner || !isReady(room)) {
    clearTurnTimer(room);
    return;
  }
...
function handleTurnTimeout(room, revision) {
  ...
  if (!room.turnDeadline || Date.now() < room.turnDeadline) {
    const remaining = Math.max(5, room.turnDeadline - Date.now());
    room.turnTimer = setTimeout(() => handleTurnTimeout(room, revision), remaining + 10);
    return;
  }
  const skipped = room.game.players[room.game.turn]?.id;
  room.game = skipCurrentTurn(room.game);
  room.lastActivityAt = Date.now();
  armTurnTimer(room, true);
...
function createRoom(config) {
  ...
  const players = {};
  for (const p of game.players) players[p.id] = null;
  const now = Date.now();
  const room = {
    code,
    createdAt: now,
...
app.get('/api/health', (req, res) => {
  ...
  res.json({ ok: true, service: 'GridGate', version: APP_VERSION, backend: BACKEND, roomsInMemory: rooms.size, ...requestMeta(req) });
});

app.post('/api/rooms', (req, res) => {
  const config = getGameConfig(req.body?.playerCount, req.body?.mode);
  const room = createRoom(config);
  res.status(201).set('cache-control', 'no-store').json({ ok: true, code: room.code, config, ...requestMeta(req) });
});
...
app.get('/api/rooms/:code', (req, res) => {
  ...
  const online = presence(room);
  const seats = {};
  for (const id of seatIds(room)) seats[id] = Boolean(room.players[id]);
  res.json({
    ok: true,
    exists: true,
    roomCode: room.code,
    ...
    connected: online,
    ready: isReady(room),
    config: { playerCount: room.game.playerCount, mode: room.game.mode, boardSize: room.game.boardSize, wallsPerPlayer: room.game.wallsPerPlayer },
    turnDeadline: room.turnDeadline,
    serverNow: Date.now(),
    backend: BACKEND,
    ...requestMeta(req),
...
app.post('/api/rooms/:code/join', (req, res) => {
  ...
  if (!seat) return res.status(409).json({ ok: false, error: 'Room is full or seats are still reserved.', stage: 'join', retryAfterMs: retryAfterMs(room), connected: presence(room), backend: BACKEND, ...requestMeta(req) });
  room.players[seat.playerId].joinReservedAt = Date.now();
  room.lastActivityAt = Date.now();
  res.json({ ok: true, stage: 'join', playerId: seat.playerId, token: seat.token, roomCode: room.code, config: { playerCount: room.game.playerCount, mode: room.game.mode }, connected: presence(room), serverNow: Date.now(), backend: BACKEND, ...requestMeta(req) });
});
...
server.on('upgrade', (req, socket, head) => {
  ...
  const seat = room ? findSeatByToken(room, token) : null;
  if (!room || !seat) {
  ...
  wss.handleUpgrade(req, socket, head, (ws) => {
    ws.gridgate = { roomCode: code, playerId: seat.playerId, token: seat.token };
    wss.emit('connection', ws, req);
  });
});

wss.on('connection', (ws) => {
  const { roomCode, playerId, token } = ws.gridgate;
  const room = rooms.get(roomCode);
  if (!room || room.players[playerId]?.token !== token) return ws.close(4003, 'Invalid session');

  const old = room.sockets.get(playerId);
  if (old && old !== ws && old.readyState === WebSocket.OPEN) old.close(4001, 'Reconnected from another tab');
  room.sockets.set(playerId, ws);
  const player = room.players[playerId];
  player.disconnectedAt = null;
  player.connectedAt = Date.now();
  player.joinReservedAt = null;
  room.lastActivityAt = Date.now();

  send(ws, { type: 'welcome', playerId, token, roomCode, serverNow: Date.now(), backend: BACKEND });
  armTurnTimer(room, false);
  send(ws, { type: 'state', ...snapshot(room, playerId) });
  broadcastState(room);
  ...
  ws.on('message', (raw) => {
    room.lastActivityAt = Date.now();
    let data;
    try { data = JSON.parse(raw.toString()); } catch { return send(ws, { type: 'error', error: 'Invalid message.' }); }
    ...
    if (data.type === 'action') {
      if (!isReady(room)) return send(ws, { type: 'error', error: 'Waiting for all players to be connected.' });
      if (room.turnDeadline && Date.now() >= room.turnDeadline) {
        handleTurnTimeout(room, room.timerRevision);
        return send(ws, { type: 'error', error: 'Your turn already expired.' });
      }
      const result = applyAction(room.game, playerId, data.action);
      if (!result.ok) return send(ws, { type: 'error', error: result.error });
      room.game = result.state;
      room.rematchVotes = [];
      if (room.game.winner) clearTurnTimer(room); else armTurnTimer(room, true);
      broadcastState(room);
      return;
    }
    ...
    if (data.type === 'rematch') {
      ...
      if (seatIds(room).every((id) => room.rematchVotes.includes(id))) {
        room.game = createInitialState({ playerCount: room.game.playerCount, mode: room.game.mode });
        room.rematchVotes = [];
        armTurnTimer(room, true);
      }
      broadcastState(room);
  ...
  ws.on('close', () => {
    ...
    if (room.sockets.get(playerId) === ws) {
      ...
      if (p?.token === token) p.disconnectedAt = Date.now();
      clearTurnTimer(room);
      broadcastState(room);
    }
  });
...
const cleanup = setInterval(() => {
  ...
  for (const [code, room] of rooms) {
    if (room.sockets.size === 0 && now - room.lastActivityAt > ROOM_IDLE_TTL_MS) {
      clearTurnTimer(room);
      rooms.delete(code);
    }
  }
...
server.listen(PORT, HOST, () => {
  console.log(`GridGate ${APP_VERSION} Render backend listening on http://${HOST}:${PORT}`);
});

function shutdown(signal) {
  console.log(`${signal}: closing GridGate server`);
